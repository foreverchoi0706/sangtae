...
- 과제 난이도: 기본 요구사항은 명확하고 직관적이어 적절한 난이도라고 생각했습니다만, 선택 과제에서 비동기를 담은 아톰의 파생 상태 구현에 난이도가 있다고 느꼈습니다. 특히 파생 상태의 Promise 처리 밑 구독자 최신화 구현이 어려웠습니다.
- 제출은 하지 않았지만 추가로 고려 중인 항목:
  - key 값을 파라미터로 추가하게 된다면 캐시 매니징을 통한 자동 업데이트 등을 고려해볼 수 있을 것 같습니다.
  - 서버사이드용 함수를 추가로 구현하여 서버에서 생성한 Atom을 서버 -> 클라이언트에 동기화 시킬 수도 있을 것같습니다.
- 선택 요구사항 외 추가 구현: `getAsync` 헬퍼를 제공해 `createAsyncAtom`으로 생성한 Atom도 try...catch없이 async/await로 소비할 수 있도록 했습니다. 이를 통해 React Suspense가 없는 환경에서도 동일 로직을 재사용할 수 있고, `Promise`를 직접 다루지 않아도 되므로 호출부 복잡도를 낮춥니다.
