...
const TODO_TEMPLATES: Array<Pick<RemoteTodo, "title" | "description">> = [
  {
    title: "API 문서 검토",
...
const loadRemoteTodos = (version: number): Promise<RemoteTodo[]> =>
  new Promise((resolve) => {
    ...
    setTimeout(() => {
      const now = new Date();
      const generated = TODO_TEMPLATES.map((template, index) => ({
...
export const createRemoteInsightsAtom = (
  ...
  createDerivedAtom<RemoteAsyncInsights>((get) => {
